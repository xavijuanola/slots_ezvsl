    ...
    with torch.no_grad():
        for batch_idx, (image, spec, bboxes, filename) in enumerate(subset_loader):
            ...
            if args.slots_maxsim == 'True':
                ...
                img_slot_out['debug_dots_sorted'] = torch.einsum('bnm,bmc->bnc', P_i, img_slot_out['debug_dots'].squeeze(1)).unsqueeze(1)
                aud_slot_out['debug_dots_sorted'] = torch.einsum('bnm,bmc->bnc', P_a, aud_slot_out['debug_dots'].squeeze(1)).unsqueeze(1)
                
                img_slot_out['debug_attn_pre_norm_sorted'] = torch.einsum('bnm,bmc->bnc', P_i, img_slot_out['debug_attn_pre_norm'].squeeze(1)).unsqueeze(1)
                aud_slot_out['debug_attn_pre_norm_sorted'] = torch.einsum('bnm,bmc->bnc', P_a, aud_slot_out['debug_attn_pre_norm'].squeeze(1)).unsqueeze(1)
                
                img_slot_out['debug_attn_sorted'] = torch.einsum('bnm,bmc->bnc', P_i, img_slot_out['debug_attn'].squeeze(1)).unsqueeze(1)
                aud_slot_out['debug_attn_sorted'] = torch.einsum('bnm,bmc->bnc', P_a, aud_slot_out['debug_attn'].squeeze(1)).unsqueeze(1)
                ...
                if args.wandb == 'True' and args.log_debug_attentions == 'True':
                    ...
                    if 'debug_attn_pre_norm_sorted' in img_slot_out:
                        for it_idx, attn_pre_norm in enumerate(img_slot_out['debug_attn_pre_norm_sorted']):
                            attn_pre_norm_np = attn_pre_norm[0].reshape(attn_pre_norm[0].shape[0], 7, 7).cpu().numpy()  # (num_slots, 7, 7)
                    ...
                    if 'debug_attn_sorted' in img_slot_out:
                        for it_idx, attn in enumerate(img_slot_out['debug_attn_sorted']):
                            attn_np = attn[0].reshape(attn[0].shape[0], 7, 7).cpu().numpy()  # (num_slots, 7, 7)
